 ...
 * @returns map结果
 */
export function arrayToMap<T extends Record<string, any>, K extends T[A], A extends keyof T>(arr: T[], attrName: A, merge?: (item1: T, item2: T) => T): MapWrap<K, T> {
 ...
 * @returns 分组后的结果
 */
export function arrayGroup<T extends Record<any, any>, K extends T[A], A extends keyof T>(arr: T[], attrName: A): MapWrap<K, T[]> {
  let result = new MapWrap<K, T[]>();
  for (let item of arr) {
    const key = item[attrName];
