...
function init(id: number) {
  ...
  // 获取内容
  const index = useTodoItemStore().items.find(e => e.id === id);
  if (index) {
    item.value = index;
  }
}
...
function updateTitle() {
  useTodoItemStore().updateById(item.value.id, {title: item.value.title})
    .catch(e => MessageUtil.error("更新标题失败", e));
}

function updatePriority(priority: any) {
  useGlobalStore().startLoading("开始更新待办项");
  useTodoItemStore().updateById(itemId.value, {priority})
