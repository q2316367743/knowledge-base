<template>
    <div class="he-editor">
        ...
        <header class="header">
            ...
            <a-button-group type="text">
                <a-space>
                    ...
                    <a-button @click="extraVisible = true" :loadin="saveLoading">
                        <template #icon>
                            <icon-settings/>
                        </template>
                    </a-button>
        ...
        <!-- 额外信息 -->
        <a-drawer v-model:visible="extraVisible" title="信息" :width="300" :footer="false">
...
import {
    ArticleIndex,
    ArticleSource,
    getDefaultArticleBaseByBaseSetting,
...
const articleIndex = ref<ArticleIndex>(getDefaultArticleIndex());
...
async function _init(articleId: number) {
    const index = useArticleStore().articleMap.get(id.value);
    if (!index) {
